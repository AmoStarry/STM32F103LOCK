...
void EXTI15_10_IRQHandler(void)
{
	...
	{
		ScanStatus = 1;
          OLED_ShowString(9,1,"ScanStatus");
...
//		/*这行代码的目的是将从USART接收的字符串数据转换为一个整数，并存储在变量 PageID 中。
//          这在处理数字字符表示的数据时很常见，比如接收到的是一个表示页码或ID的数字字符串，需要将其转换为可以在程序中使用的整数形式*/
//          
